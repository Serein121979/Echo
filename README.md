...
> `Clips` 子系统中文说明：见 [`CLIPS_ZH.md`](./CLIPS_ZH.md)
...
- 收藏 / 归档：支持主列表、收藏、归档三种视图切换
- 附件消息：支持粘贴图片、选择文件、跨端下载附件
- 消息删除：软删除实现，便于后续扩展撤销 / 归档
- 消息编辑：编辑后自动刷新自动标签
- 全文搜索：优先使用 PostgreSQL FTS，旧表结构自动降级为前端匹配
- 消息列表：自动滚动到底部，体验接近 IM
- 移动端体验：侧边栏默认收起，使用抽屉方式展开，界面为黑白极简风格
- 视觉结构：消息列表和 composer 直接贴页面背景，不再使用整块白色外层卡片
- 交互结构：composer 采用图标化单行布局，消息操作通过三点按钮展开
- 安装体验：补充了原生 PWA manifest，可安装到桌面或主屏幕
- 分享目标：安装后可作为系统分享目标，文字、URL、图片和文件可直接进入收件箱
...
- 为 `notes` 增加 `is_starred`
- 为 `notes` 增加 `is_archived`
- 为 `notes` 增加附件字段（`file_path` / `file_url` / `file_name` / `file_type` / `file_size`）
- 为 `notes` 增加全文搜索向量列 `fts`
- 写入默认标签与默认自动标签规则
- 创建公开附件桶 `echo-files` 及对应策略
- 为软删除和全文搜索创建索引
- 创建默认文件夹 `收件箱`
- 配置 RLS 策略
...
- 如果手机访问开发服务器不稳定，请确认 `next.config.ts` 里的 `allowedDevOrigins` 已包含当前电脑的局域网 IP
...
**Q：自动标签规则可以自己改吗？**  
可以。执行最新 `supabase/schema.sql` 后，左侧会出现自动标签规则面板，支持新增、编辑、删除规则。

**Q：收藏和归档有什么区别？**  
收藏用于标记重要消息；归档用于把已处理内容从主列表移开。你可以在“全部 / 收藏 / 归档”视图之间切换。

**Q：现在可以发图片和文件了吗？**  
可以。发送框支持粘贴图片，也支持选择文件；发送后会作为附件消息保存，电脑和手机两端都可以直接下载。

**Q：可以从手机系统分享内容到 Echo 吗？**  
可以。先把 Echo 安装到主屏幕或桌面，然后在支持分享的 App 里选择 Echo，文字、URL 和图片会直接进入收件箱。

**Q：为什么列表和输入框看起来没有白色大卡片了？**  
这是当前的极简布局设定。消息列表和 composer 直接贴在页面背景上，只保留消息气泡和输入控件自身的轻量样式。

**Q：消息操作为什么只剩三点按钮？**  
为了更接近文件传输助手式的极简聊天体验，消息的收藏、归档、编辑、删除都收进了三点按钮，点击后再展开。

**Q：手机端怎么下载附件？**  
打开消息里的“下载文件”按钮即可，不需要再单独打开原始链接。
